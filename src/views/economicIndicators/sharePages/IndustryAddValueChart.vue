...
export default {
  ...
  methods: {
    ...
    async getMaxMinDate() {
      // 获取最大年最小年
      let res = await chartDataFun.getMaxMinDate("GIVA");
      console.log(res);
    ...
    async getItemCategoryData(res, XNameAttr, dataAttr, range) {
      ...
      this.USD.series[0]["data"] = data.yoyGrowth;
      this.USD.series[1]["data"] = data.yoyCumulativeGrowth;
    },
    async getChartsData(aug) {
      //改变横轴 获取数据
      let { res } = await request.getIndustryAddValueChartsData(aug);
      // 完整的区间
      let range = await chartDataFun.getXRangeMC(aug);
      // 要换取纵轴数据的字段属性
      let dataAttr = ["yoyGrowth", "yoyCumulativeGrowth"];
      let XNameAttr = "M";
